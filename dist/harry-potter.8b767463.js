...
},{"d3-collection":"../node_modules/d3-collection/src/index.js","d3-selection":"../node_modules/d3-selection/src/index.js"}],"data/harrypotter.csv":[function(require,module,exports) {
module.exports = "/harrypotter.88bb967c.csv";
},{}],"scripts/harry-potter.js":[function(require,module,exports) {
"use strict";
...
var svg = d3.select('#harry-potter').append('svg').attr('height', height + margin.top + margin.bottom).attr('width', width + margin.left + margin.right).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')'); // Create our scales
...
var tip = d3.tip().attr('class', 'd3-tip').offset([-10, 0]).html(function (d) {
  return " <span style='color:black'>".concat(d.values.gender, "</span>");
});
svg.call(tip); // Read in files

d3.csv(require('../data/harrypotter.csv')).then(ready).catch(function (err) {
  console.log(err);
});
...
function ready(datapoints) {
  var nested = d3.nest().key(function (d) {
    return d.name;
  }).entries(datapoints); // Go through each group, pull out the name of that group

  var names = nested.map(function (d) {
    return d.key;
  }); // Teach it to the y position scale

  yPositionScale.domain(names); // Add a g element for every single city
  // and then let's do something with each of you

  svg.selectAll('g').data(nested).enter().append('g').attr('transform', function (d) {
    return "translate(0,".concat(yPositionScale(d.key), ")");
  }).each(function (d) {
    var g = d3.select(this);
    var name = d.key;
    var datapoints = d.values;
    var one = d3.max(datapoints, function (d) {
      return d.book1;
    });
    var two = d3.max(datapoints, function (d) {
      return d.book2;
    });
    var three = d3.max(datapoints, function (d) {
      return d.book3;
    });
    var max = d3.max(datapoints, function (d) {
      return d.values;
    });
    g.append('line').attr('y1', 0).attr('y2', 0).attr('x1', xPositionScale(one)).attr('x2', xPositionScale(two)).attr('stroke', 'gray').attr('fill', 'blue');
    g.append('circle').attr('r', 7).attr('fill', 'lightblue').attr('cy', 0).attr('cx', xPositionScale(one)).on('mouseover', tip.show).on('mouseout', tip.hide);
    g.append('circle').attr('r', 7).attr('fill', 'blue').attr('cy', 0).attr('cx', xPositionScale(two));
  });
...
},{"d3":"../node_modules/d3/index.js","d3-tip":"../node_modules/d3-tip/index.js","../data/harrypotter.csv":"data/harrypotter.csv"}]},{},["scripts/harry-potter.js"], null)
//# sourceMappingURL=/harry-potter.8b767463.js.map